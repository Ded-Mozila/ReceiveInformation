...
void statisticsFun(const VVfloat&,const VVfloat&,const string&,int);//Основа для всех функций 
...
float Variance(const vector<float>&,const float);					//Дисперсия
float StandardDeviation(const vector<float>&,float);				//Среднеквадратическое отклонение
#endif
